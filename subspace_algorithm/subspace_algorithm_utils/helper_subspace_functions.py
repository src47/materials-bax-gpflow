...
def obtain_discrete_pareto_optima(x=None, y=None, error_bars=None, use_errors=True):
    ...
    while next_point_index < len(y):
        nondominated_point_mask = np.any(
            y >= y[next_point_index] - error_bars[next_point_index], axis=1
        )
...
def wish_list(y_vals, list_of_wishes):
    """
    Based on Multiregion intersection
    """
    y_vals_array = np.array(y_vals)
    nwishes = len(list_of_wishes)
    desired_pts = set()
    for wish in list_of_wishes:
        desired_pts_tmp = multi_level_region_intersection_Nd(y_vals_array, wish)
        desired_pts = desired_pts.union(set(desired_pts_tmp))

    return list(desired_pts)
...
def multi_level_region_union_Nd(y_vals_array, thresholds_list):
    ...
    assert y_vals_array.shape[1] == len(
        thresholds_list
    ), "As many threshold lists as y_vals_array"
...
def multi_level_region_intersection_Nd(y_vals_array, thresholds_list):
    ...
    for i in range(n_dims_y):
        ...
        if i == 0:
            ...
            desired_points = desired_points.intersection(
                set(level_region_1d(y_vals, thresholds))
            )
...
def discontinous_library_1d(y_vals, list_of_vals, eps_vals: List[float] = None):
    ...
    assert len(list_of_vals) == len(
        eps_vals
    ), "eps_vals must be same size as list_of_vals"
...
def convert_y_for_optimization(y, max_or_min_list):
    ...
    assert np.array_equal(
        max_or_min_list, max_or_min_list.astype(bool)
    ), "The array is not boolean."
    assert y.shape[1] == len(
        max_or_min_list
    ), "Please note whether each property should be maximized or minimized"
