...
class GpflowGp(SimpleGp):
    ...
    def __init__(self, params=None, hyperparameter_params=None, data=None, verbose=False):
        """
        Parameters
        ----------
    ...
    def set_params(self, params):
        ...
        # hyperparameter_params = dict_to_namespace(hyperparameter_params)
        ...
        self.params.fixed_mean_func = getattr(params, "fixed_mean_func", False)
        self.params.mean_func_c = getattr(params, "mean_func_c", 0.0)
        self.params.ls = getattr(params, "ls", 0.5)
        self.params.use_ard = getattr(params, "use_ard", True)
        self.params.alpha = getattr(params, "alpha", 0.5)
        self.params.fixed_noise = getattr(params, "fixed_noise", True)
        self.params.sigma = getattr(params, "sigma", 0.001)

        # hyperparameter_params.ls_lower_bound
        # hyperparameter_params.ls_upper_bound
        # hyperparameter_params.alpha_lower_bound
        # hyperparameter_params.alpha_upper_bound
    ...
    def build_new_gpflow_model_on_data(self, data):
        ...
        gpflow_data = (np.array(data.x).reshape(-1, n_dimx), np.array(data.y).reshape(-1, 1))
        ...
        if self.params.use_ard:
            ...
            ls_init = self.params.ls
        kernel = gpflow.kernels.SquaredExponential(variance=self.params.alpha**2, lengthscales=ls_init)
    ...
    def fit_hypers(self):
        ...
        opt_log = opt.minimize(self.model.training_loss, self.model.trainable_variables, options=opt_config)
        if self.params.print_fit_hypers:
